...
- **`src/graph-data.ts`** — The entire dataset: 244 service nodes, 301 typed edges, and 17 life events. This is the primary file to edit when adding/modifying services or relationships.
...
- `REQUIRES` — strict ordering constraint (prerequisite must be completed first). Only 21 of the 301 edges are `REQUIRES`, and they are the only ones that affect phase ordering, so they carry the most weight and deserve the most scrutiny.
